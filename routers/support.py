...
async def start_support(message: types.Message, state: FSMContext):
    ...
    await message.answer("Please describe your problem. We will respond as soon as possible.")
...
async def process_question(message: types.Message, state: FSMContext):
    ...
    builder.button(text="To answer", callback_data=f"reply_{message.from_user.id}")
    ...
    await message.answer("Your question has been sent to support. Expect a response.")
    await state.clear()
...
async def admin_reply(message: types.Message):
    ...
    await bot.send_message(
        user_id,
        f"Response from support:\n\n{message.text}"
    )
    
    await message.answer("Your response has been sent to the user.")
...
async def process_reply_button(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(f"Enter the response for the user {user_id}:")
